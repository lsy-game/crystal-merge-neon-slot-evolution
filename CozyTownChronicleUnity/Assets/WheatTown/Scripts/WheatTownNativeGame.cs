...
namespace WheatTown
{
    public sealed class WheatTownNativeGame : MonoBehaviour
    {
        ...
        private enum TaskTab { Orders, Album, DailyRoute }
        private enum SfxKind { Click, Open, Success, Error, Plant, HarvestStart, Reward }
        ...
        private RectTransform harvestFxLayer;
        private RectTransform harvestMachineRect;
        private RectTransform harvestResultPanel;
        private Coroutine harvestFxRoutine;
        ...
        private Sprite authTitlePlaqueRuntime;
        ...
        private const string TutorialDoneKey = "WheatTownGuest.tutorialDone";
        ...
        private AudioClip sfxHarvestStart;
        private AudioClip sfxReward;
        ...
        private void SetupAudio()
        {
            ...
            sfxClick = CreateToneClip("WheatTownClick", 880f, 1180f, .055f, .16f, Wave.Sine);
            sfxOpen = CreateToneClip("WheatTownOpen", 520f, 780f, .12f, .20f, Wave.Sine);
            sfxSuccess = CreateToneClip("WheatTownSuccess", 660f, 1040f, .18f, .24f, Wave.Triangle);
            sfxError = CreateToneClip("WheatTownError", 260f, 160f, .16f, .22f, Wave.Square);
            sfxPlant = CreateToneClip("WheatTownPlant", 420f, 700f, .16f, .22f, Wave.Triangle);
            sfxHarvestStart = CreateToneClip("WheatTownHarvestStart", 320f, 980f, .22f, .18f, Wave.Saw);
            sfxReward = CreateToneClip("WheatTownReward", 740f, 1320f, .28f, .26f, Wave.Triangle);
        }
        ...
        private void BuildHud()
        {
            ...
            AddReadableBorder(topResourceBar, new Vector2(430, 62), 1.5f, C(255, 229, 154, 80));
            ...
            AddReadableBorder(bottomNavBar, new Vector2(430, 78), 1.5f, C(255, 229, 154, 95));
        ...
        private Button NavButton(RectTransform parent, string label, Sprite icon, float x, UnityEngine.Events.UnityAction action)
        {
            ...
            AddReadableBorder(button.GetComponent<RectTransform>(), new Vector2(88, 66), 1.5f, C(255, 236, 176, 82));
        ...
        private void RebuildAuthContent()
        {
            ...
            var titlePlate = Panel(authLayer, "AuthTitlePlate", Color.clear, new Vector2(0, 308), new Vector2(362, 124), null);
            ImageNode(titlePlate, "PlaqueArt", AuthTitlePlaque(), Vector2.zero, new Vector2(360, 118), Color.white);
            ImageNode(titlePlate, "TitleWheat", wheatIcon, new Vector2(0, 35), new Vector2(46, 46), C(255, 246, 205, 255));
            var title = TextNode(titlePlate, "GameTitle", "Wheat Town", new Vector2(0, 5), new Vector2(310, 42), 32, C(255, 249, 224, 255), TextAnchor.MiddleCenter);
            title.fontStyle = FontStyles.Bold;
            var subtitlePlate = Panel(titlePlate, "SubtitlePlate", C(21, 57, 38, 218), new Vector2(0, -36), new Vector2(308, 28), null);
            AddReadableBorder(subtitlePlate, new Vector2(308, 28), 1.5f, C(255, 229, 154, 120));
            TextNode(subtitlePlate, "Subtitle", "Cozy Harvest · Town Life · Casual Growth", Vector2.zero, new Vector2(286, 22), 14, C(255, 240, 202, 255), TextAnchor.MiddleCenter);

            var card = Panel(authLayer, "LoginCard", C(255, 246, 226, 255), new Vector2(0, -82), new Vector2(372, 612), AuthLoginCard());
        ...
        private void BuildTownPage()
        {
            ...
            for (var i = 0; i < 6; i++)
            {
                var col = i % 2;
                var row = i / 2;
                var pos = new Vector2(-146 + col * 98, 230 - row * 84);
                PlotButton(i, pos);
            }
        ...
        private void PlotButton(int index, Vector2 pos)
        {
            ...
            var button = ButtonNode(townPage, "Plot" + index, "", pos, new Vector2(88, 66), C(255, 255, 255, 1), () => ClickPlot(index), null);
            var buttonImage = button.GetComponent<Image>();
            if (buttonImage != null) buttonImage.color = C(255, 255, 255, 1);
            var tile = BakedFieldTile(index, state);
            if (tile != null)
            {
                var fieldTile = ImageNode(button.transform, "FieldTile", tile, Vector2.zero, new Vector2(88, 66), Color.white);
                var tileImage = fieldTile.GetComponent<Image>();
                if (tileImage != null) tileImage.preserveAspect = false;
            }
            if (state == PlotState.Locked)
            {
                ImageNode(button.transform, "Lock", lockIcon, new Vector2(0, 0), new Vector2(30, 30), C(255, 255, 255, 230));
            }
        ...
        private Sprite BakedFieldTile(int index, PlotState state)
        {
            switch (state)
            {
                case PlotState.GrowingOne:
                    return ResourceBaked("field-baked-v14", index, "seeded");
                case PlotState.GrowingTwo:
                    return ResourceBaked("field-baked-v14", index, "young_wheat");
                case PlotState.ReadyWheat:
                    return ResourceBaked("field-baked-v14", index, "wheat");
                case PlotState.Empty:
                case PlotState.Locked:
                default:
                    return ResourceBaked("field-baked-v14", index, "empty");
            }
        }
        ...
        {
            return ResourceBaked("field-baked-v11", index, state);
        }

        private Sprite ResourceBaked(string folder, int index, string state)
        {
            var key = "WheatTown/" + folder + "/plot_" + index + "_" + state;
        ...
        private void SceneHotspot(string label, Vector2 pos, Vector2 size, UnityEngine.Events.UnityAction action, bool attention)
        {
            ...
            AddReadableBorder(tapPlate, tapPlate.sizeDelta, 1.5f, C(255, 229, 154, 135));
        ...
        private void BuildBagPage()
        {
            ...
            for (var i = 0; i < 12; i++)
            {
                ...
                if (i < items.Count)
                {
                    ...
                    AddReadableBorder(countBadge, new Vector2(88, 26), 1f, C(255, 231, 157, 70));
            ...
            AddReadableBorder(hint, new Vector2(334, 36), 1.5f, C(181, 137, 74, 150));
        ...
        private void BuildTaskPage()
        {
            ...
            TaskTabButton(header, "Album", TaskTab.Album, 48);
            ...
            if (activeTaskTab == TaskTab.Orders)
            {
                ...
                TextNode(panel, "OrderHint", "Orders give coins and materials. Resident requests stay on the town map.", new Vector2(0, -104), new Vector2(304, 64), 16, C(70, 64, 52, 255), TextAnchor.MiddleCenter);
            }
            else if (activeTaskTab == TaskTab.Album)
            {
                TextNode(panel, "PanelTitle", "Town Album", new Vector2(0, 210), new Vector2(250, 38), 24, C(255, 248, 222, 255), TextAnchor.MiddleCenter);
                ImageNode(panel, "BookIcon", TaskCollectionBook(), new Vector2(-128, 210), new Vector2(34, 32), Color.white);
                CollectionRow(panel, "Wheat Album", wheatIcon, dailyHarvest, 18, new Vector2(0, 128));
                CollectionRow(panel, "Bread Baking", breadIcon, bread, 3, new Vector2(0, 62));
                CollectionRow(panel, "Friend Favor", giftIcon, miaFavor + tomFavor, 10, new Vector2(0, -4));
                var tip = Panel(panel, "AlbumTipBox", C(255, 246, 219, 248), new Vector2(0, -98), new Vector2(314, 74), null);
                AddReadableBorder(tip, new Vector2(314, 74), 2f, C(181, 137, 74, 180));
                TextNode(tip, "Tip", "Album progress gives one-time town-growth resources. Mia and Tom requests are opened from their map houses.", Vector2.zero, new Vector2(286, 58), 15, C(73, 63, 48, 255), TextAnchor.MiddleCenter);
            }
            else
            {
            ...
            AddReadableBorder(hint, new Vector2(334, 38), 1.5f, C(181, 137, 74, 150));
        ...
        {
            if (tab == TaskTab.Album) return "Album";
        ...
        private void BuildHarvestPage()
        {
            ...
            ImageNode(summary, "WheatMark", wheatIcon, new Vector2(-172, 8), new Vector2(30, 30), C(255, 245, 205, 255));
            TextNode(summary, "Title", "Harvest", new Vector2(-94, 12), new Vector2(138, 34), 26, C(32, 57, 39, 255), TextAnchor.MiddleLeft);
            TextNode(summary, "Subtitle", "3 matching symbols win. Clear 5 x 3 table.", new Vector2(-6, -22), new Vector2(286, 26), 15, C(75, 67, 52, 255), TextAnchor.MiddleCenter);
            ...
            var machine = Panel(harvestPage, "HarvestMachine", C(255, 250, 232, 255), new Vector2(0, -24), new Vector2(376, 548), null);
            harvestMachineRect = machine;
            ...
            harvestResultPanel = resultPanel;
            ...
            harvestFxLayer = Panel(machine, "HarvestFxLayer", Color.clear, Vector2.zero, new Vector2(376, 548), null);
            harvestFxLayer.SetAsLastSibling();

            var back = ButtonNode(harvestPage, "BackTown", "Back to Town", new Vector2(0, -372), new Vector2(224, 52), C(255, 250, 232, 255), () => ShowHarvest(false), null);
        ...
        private void OpenResident(string resident)
        {
            ...
            var materialName = isMia ? "Wheat" : "Bread";
            var materialIcon = isMia ? wheatIcon : breadIcon;
            var owned = isMia ? wheat : bread;
            var required = isMia ? 2 : 1;
            var favor = isMia ? miaFavor : tomFavor;
            ShowResidentPanel(resident, isMia ? NpcMia() : NpcTom(), materialIcon, materialName, owned, required, favor, done, () =>
            {
                if (done)
                {
                    HideModal();
        ...
        private IEnumerator HarvestSpinSequence()
        {
            ...
            if (harvestFxLayer != null) Clear(harvestFxLayer);
            if (harvestResultPanel != null) harvestResultPanel.localScale = Vector3.one;
            ...
            for (var col = 0; col < 5; col++)
            {
                for (var row = 0; row < 3; row++)
                {
                    ...
                    if (cellIndex < reelCells.Count && reelCells[cellIndex] != null)
                    {
                        yield return StartCoroutine(PlayCellLandingFx(reelCells[cellIndex], col));
                    }
                }
            }
            ...
            if (harvestFxRoutine != null) StopCoroutine(harvestFxRoutine);
            harvestFxRoutine = StartCoroutine(PlayHarvestSettlementFx(reward, winners, helpBonus, results));
            yield return harvestFxRoutine;
            harvestFxRoutine = null;
            PlaySfx(reward > 0 || helpBonus ? SfxKind.Reward : SfxKind.Success);
        ...
        private IEnumerator HighlightHarvestWinners(List<(string Name, Sprite Sprite, int Weight, int Pay)> results, HashSet<string> winners)
        {
            ...
            {
                for (var i = 0; i < reelCells.Count; i++) SetHarvestCellVisual(i, 1f, C(255, 253, 242, 255), C(255, 239, 164, 36));
            ...
            for (var pulse = 0; pulse < 3; pulse++)
            {
                ...
                {
                    var winning = winners.Contains(results[i].Name);
                    SetHarvestCellVisual(i, winning ? 1.15f : .92f, winning ? C(255, 244, 186, 255) : C(232, 222, 202, 235), winning ? C(255, 206, 74, 150) : C(255, 239, 164, 18));
                }
                yield return new WaitForSeconds(0.11f);
                for (var i = 0; i < reelCells.Count; i++)
                {
                    SetHarvestCellVisual(i, 1f, C(255, 253, 242, 255), C(255, 239, 164, 36));
                }
                yield return new WaitForSeconds(0.08f);
            }
        }

        private void SetHarvestCellVisual(int index, float scale, Color cellColor, Color glowColor)
        {
            if (index < 0 || index >= reelCells.Count || reelCells[index] == null) return;
            var cell = reelCells[index];
            cell.localScale = Vector3.one * scale;
            var cellImage = cell.GetComponent<Image>();
            if (cellImage != null) cellImage.color = cellColor;
            var glow = cell.Find("IconGlow") != null ? cell.Find("IconGlow").GetComponent<Image>() : null;
            if (glow != null) glow.color = glowColor;
        }

        private IEnumerator PlayCellLandingFx(RectTransform cell, int column)
        {
            if (cell == null) yield break;
            var start = cell.anchoredPosition + new Vector2(0, 8 + column * 2);
            var end = cell.anchoredPosition;
            cell.anchoredPosition = start;
            cell.localScale = Vector3.one * 1.08f;
            yield return new WaitForSeconds(0.018f);
            cell.anchoredPosition = end;
            cell.localScale = Vector3.one;
        }

        private IEnumerator PlayHarvestSettlementFx(int reward, HashSet<string> winners, bool helpBonus, List<(string Name, Sprite Sprite, int Weight, int Pay)> results)
        {
            if (harvestFxLayer == null)
            {
                yield break;
            }

            Clear(harvestFxLayer);
            var hasWin = reward > 0;
            var bigWin = reward >= 45;
            var title = helpBonus ? "Help Energy!" : bigWin ? "Great Harvest!" : hasWin ? "Harvest Win!" : "Keep Growing";
            var subtitle = hasWin ? "+" + reward + " Coins" : "Wood +1";

            var originalMachinePos = harvestMachineRect != null ? harvestMachineRect.anchoredPosition : Vector2.zero;
            Panel(harvestFxLayer, "WinWash", hasWin ? C(255, 226, 117, 56) : C(37, 76, 48, 22), new Vector2(0, 76), new Vector2(360, 304), null);
            if (hasWin)
            {
                for (var i = 0; i < 12; i++)
                {
                    var ray = Panel(harvestFxLayer, "GoldRay" + i, C(255, 220, 105, bigWin || helpBonus ? (byte)82 : (byte)48), new Vector2(0, 108), new Vector2(10, bigWin || helpBonus ? 214 : 154), null);
                    ray.pivot = new Vector2(.5f, 0f);
                    ray.localRotation = Quaternion.Euler(0, 0, i * 30f);
                }
            }

            var titleText = TextNode(harvestFxLayer, "WinTitle", title, new Vector2(0, 168), new Vector2(300, 48), bigWin || helpBonus ? 30 : 24, C(255, 248, 222, 255), TextAnchor.MiddleCenter);
            titleText.outlineColor = C(70, 45, 18, 230);
            titleText.outlineWidth = .08f;
            var rewardText = TextNode(harvestFxLayer, "RewardText", subtitle, new Vector2(0, 126), new Vector2(260, 42), bigWin || helpBonus ? 28 : 22, hasWin ? C(255, 223, 95, 255) : C(255, 248, 222, 255), TextAnchor.MiddleCenter);
            rewardText.outlineColor = C(70, 45, 18, 230);
            rewardText.outlineWidth = .08f;

            var plate = Panel(harvestFxLayer, "WinPlate", C(72, 46, 20, hasWin ? (byte)178 : (byte)92), new Vector2(0, 130), new Vector2(bigWin || helpBonus ? 270 : 210, 84), null);
            plate.SetAsFirstSibling();
            AddReadableBorder(plate, plate.sizeDelta, 2f, C(255, 220, 120, hasWin ? (byte)220 : (byte)120));

            var titleRect = titleText.GetComponent<RectTransform>();
            var rewardRect = rewardText.GetComponent<RectTransform>();
            var burstCount = helpBonus ? 34 : bigWin ? 42 : hasWin ? 24 : 7;
            var burstIcons = new List<Image>();
            var burstTargets = new List<Vector2>();
            for (var i = 0; i < burstCount; i++)
            {
                var iconSprite = hasWin ? coinIcon : wheatIcon;
                var icon = ImageNode(harvestFxLayer, "RewardSpark" + i, iconSprite, new Vector2(0, 102), new Vector2(hasWin ? 24 : 18, hasWin ? 24 : 18), C(255, 255, 255, hasWin ? (byte)235 : (byte)165)).GetComponent<Image>();
                burstIcons.Add(icon);
                var angle = (360f / burstCount) * i + random.Next(-12, 13);
                var distance = random.Next(hasWin ? 104 : 42, hasWin ? 194 : 86);
                burstTargets.Add(new Vector2(Mathf.Cos(angle * Mathf.Deg2Rad), Mathf.Sin(angle * Mathf.Deg2Rad)) * distance + new Vector2(0, random.Next(-18, 36)));
            }

            var winnerSparkles = new List<Image>();
            var winnerOrigins = new List<Vector2>();
            if (winners != null && winners.Count > 0 && results != null)
            {
                for (var i = 0; i < results.Count && i < reelCells.Count; i++)
                {
                    if (!winners.Contains(results[i].Name) || reelCells[i] == null) continue;
                    var origin3 = harvestFxLayer.InverseTransformPoint(reelCells[i].TransformPoint(Vector3.zero));
                    var origin = new Vector2(origin3.x, origin3.y);
                    for (var s = 0; s < (bigWin || helpBonus ? 4 : 2); s++)
                    {
                        var sparkle = Panel(harvestFxLayer, "WinnerSparkle" + i + "_" + s, C(255, 232, 112, 230), origin, new Vector2(10, 10), null).GetComponent<Image>();
                        winnerSparkles.Add(sparkle);
                        winnerOrigins.Add(origin + new Vector2(random.Next(-22, 23), random.Next(-24, 25)));
                    }
                }
            }

            var rainIcons = new List<Image>();
            var rainStarts = new List<Vector2>();
            var rainEnds = new List<Vector2>();
            if (bigWin || helpBonus)
            {
                for (var i = 0; i < 28; i++)
                {
                    var x = random.Next(-170, 171);
                    var start = new Vector2(x, random.Next(250, 340));
                    var end = new Vector2(x + random.Next(-42, 43), random.Next(-246, -112));
                    var coin = ImageNode(harvestFxLayer, "CoinRain" + i, coinIcon, start, new Vector2(20, 20), C(255, 255, 255, 230)).GetComponent<Image>();
                    rainIcons.Add(coin);
                    rainStarts.Add(start);
                    rainEnds.Add(end);
                }
            }

            var duration = bigWin || helpBonus ? .96f : .74f;
            var elapsed = 0f;
            while (elapsed < duration)
            {
                elapsed += Time.deltaTime;
                var t = Mathf.Clamp01(elapsed / duration);
                var pop = 1f + Mathf.Sin(t * Mathf.PI) * (bigWin || helpBonus ? .22f : .13f);
                titleRect.localScale = Vector3.one * pop;
                rewardRect.localScale = Vector3.one * (1f + Mathf.Sin(Mathf.Clamp01(t * 1.25f) * Mathf.PI) * .18f);
                if (harvestResultPanel != null)
                {
                    harvestResultPanel.localScale = Vector3.one * (1f + Mathf.Sin(t * Mathf.PI * 3f) * .035f);
                }
                if ((bigWin || helpBonus) && harvestMachineRect != null)
                {
                    var shake = Mathf.Sin(t * Mathf.PI * 18f) * (1f - t) * 5f;
                    harvestMachineRect.anchoredPosition = originalMachinePos + new Vector2(shake, -shake * .35f);
                }
                for (var i = 0; i < burstIcons.Count; i++)
                {
                    if (burstIcons[i] == null) continue;
                    var rect = burstIcons[i].GetComponent<RectTransform>();
                    rect.anchoredPosition = Vector2.Lerp(new Vector2(0, 102), burstTargets[i], Mathf.SmoothStep(0f, 1f, t));
                    rect.localRotation = Quaternion.Euler(0, 0, Mathf.Lerp(0f, 260f + i * 13f, t));
                    rect.localScale = Vector3.one * Mathf.Lerp(.65f, 1.2f, Mathf.Sin(t * Mathf.PI));
                    var color = burstIcons[i].color;
                    color.a = Mathf.Lerp(color.a, 0f, Mathf.Clamp01((t - .58f) / .42f));
                    burstIcons[i].color = color;
                }
                for (var i = 0; i < winnerSparkles.Count; i++)
                {
                    if (winnerSparkles[i] == null) continue;
                    var phase = Mathf.Repeat(t * 3.5f + i * .21f, 1f);
                    var rect = winnerSparkles[i].GetComponent<RectTransform>();
                    rect.anchoredPosition = winnerOrigins[i] + new Vector2(Mathf.Sin((t * 8f + i) * Mathf.PI) * 12f, phase * 26f);
                    rect.localScale = Vector3.one * Mathf.Lerp(.55f, 1.45f, Mathf.Sin(phase * Mathf.PI));
                    var color = winnerSparkles[i].color;
                    color.a = Mathf.Lerp(0f, 1f, Mathf.Sin(phase * Mathf.PI));
                    winnerSparkles[i].color = color;
                }
                for (var i = 0; i < rainIcons.Count; i++)
                {
                    if (rainIcons[i] == null) continue;
                    var phase = Mathf.Clamp01((t - i * .012f) / .75f);
                    var rect = rainIcons[i].GetComponent<RectTransform>();
                    rect.anchoredPosition = Vector2.Lerp(rainStarts[i], rainEnds[i], phase);
                    rect.localRotation = Quaternion.Euler(0, 0, phase * 360f);
                    var color = rainIcons[i].color;
                    color.a = phase >= 1f ? 0f : 1f;
                    rainIcons[i].color = color;
                }
                yield return null;
            }
            ...
            if (harvestResultPanel != null) harvestResultPanel.localScale = Vector3.one;
            if (harvestMachineRect != null) harvestMachineRect.anchoredPosition = originalMachinePos;
            yield return new WaitForSeconds(bigWin || helpBonus ? .22f : .08f);
            Clear(harvestFxLayer);
        }
        ...
        private void ShowResidentPanel(string resident, Sprite portrait, Sprite materialIcon, string materialName, int owned, int required, int favor, bool done, UnityEngine.Events.UnityAction submit)
        {
            PlaySfx(SfxKind.Open, .7f);
            modalLayer.gameObject.SetActive(true);
            Clear(modalLayer);
            ...
            var ready = owned >= required;
            var missing = Mathf.Max(0, required - owned);
            var status = done ? "Done today" : ready ? "Ready to submit" : "Need " + missing + " more " + materialName;
            var rewardText = resident == "Mia" ? "Coins +180, Favor +1" : "Ore +2, Favor +1";
            ...
            var needRow = Panel(panel, "NeedInfoRow", C(255, 246, 219, 255), new Vector2(68, 68), new Vector2(232, 42), null);
            AddReadableBorder(needRow, new Vector2(232, 42), 2f, ready || done ? C(87, 139, 82, 210) : C(190, 93, 61, 210));
            TextNode(needRow, "Label", "Need", new Vector2(-82, 0), new Vector2(58, 24), 15, C(46, 74, 43, 255), TextAnchor.MiddleLeft);
            ImageNode(needRow, "MaterialIcon", materialIcon, new Vector2(-22, 0), new Vector2(25, 25), Color.white);
            TextNode(needRow, "Amount", materialName + " " + owned + " / " + required, new Vector2(54, 0), new Vector2(116, 24), 15, ready || done ? C(43, 111, 66, 255) : C(160, 73, 49, 255), TextAnchor.MiddleLeft);
            InfoRow(panel, "Status", status, new Vector2(68, 18), 232, 15);
            InfoRow(panel, "Reward", rewardText, new Vector2(68, -32), 232, 14);
            ...
            var buttonLabel = done ? "Close" : ready ? "Submit" : "Need " + missing + " " + materialName;
            var buttonColor = done ? C(46, 74, 43, 255) : ready ? C(232, 141, 47, 255) : C(152, 139, 118, 255);
            ButtonNode(panel, "Submit", buttonLabel, new Vector2(82, -144), new Vector2(196, 52), buttonColor, done || ready ? submit : () => Toast(materialName + " " + owned + " / " + required + ". Harvest or craft more first."), null);
        }

        private void ShowSettings()
        {
            PlaySfx(SfxKind.Open, .6f);
            modalLayer.gameObject.SetActive(true);
            Clear(modalLayer);
            Panel(modalLayer, "Mask", C(0, 0, 0, 145), Vector2.zero, new Vector2(430, 932), null);
            ...
            ButtonNode(dialog, "ReplayGuide", "Replay Guide", new Vector2(0, -76), new Vector2(226, 44), C(232, 141, 47, 255), ReplayTutorialFromSettings, null);
            ButtonNode(dialog, "Privacy", "Privacy", new Vector2(-90, -132), new Vector2(150, 42), C(46, 74, 43, 255), () => ShowAgreement("Privacy"), null);
            ButtonNode(dialog, "Terms", "Terms", new Vector2(90, -132), new Vector2(150, 42), C(46, 74, 43, 255), () => ShowAgreement("Terms"), null);
            ButtonNode(dialog, "Logout", "Save & Log out", new Vector2(0, -190), new Vector2(272, 46), C(46, 74, 43, 255), LogoutToAuth, null);
            ButtonNode(dialog, "DeleteSave", "Delete Save", new Vector2(0, -246), new Vector2(246, 44), C(126, 70, 48, 255), DeleteGuestSaveFromSettings, null);
            TextNode(dialog, "NativeTip", "Local guest save only. No web page is opened.", new Vector2(0, -292), new Vector2(318, 24), 13, C(96, 84, 64, 255), TextAnchor.MiddleCenter);
        }
        ...
        {
            InfoRow(parent, label, value, pos, 232, 15);
        }

        private void InfoRow(RectTransform parent, string label, string value, Vector2 pos, float width, int valueFontSize)
        {
            var row = Panel(parent, label + "InfoRow", C(255, 246, 219, 255), pos, new Vector2(width, 40), null);
            AddReadableBorder(row, new Vector2(width, 40), 2f, C(181, 137, 74, 180));
            TextNode(row, "Label", label, new Vector2(-width * .33f, 0), new Vector2(68, 24), 15, C(46, 74, 43, 255), TextAnchor.MiddleLeft);
            var valueText = TextNode(row, "Value", value, new Vector2(34, 0), new Vector2(width - 96, 26), valueFontSize, C(73, 63, 48, 255), TextAnchor.MiddleLeft);
        ...
        private void RefreshCurrentPage()
        {
            ...
            if (tutorialActive) BuildTutorialStep();
        }
        ...
        private void Toast(string message)
        {
            ...
            if (toastBox != null)
            {
                toastBox.anchoredPosition = harvestPage != null && harvestPage.gameObject.activeSelf
                    ? new Vector2(0, -414)
                    : townPage != null && townPage.gameObject.activeSelf
                        ? new Vector2(0, -238)
                        : new Vector2(0, -350);
            }
        ...
        {
            guideLayer = Panel(root, "GuideLayer", Color.clear, Vector2.zero, new Vector2(430, 932), null);
            Stretch(guideLayer);
        ...
        {
            if (PlayerPrefs.GetInt(TutorialDoneKey, 0) == 1) return;
            StartTutorial(false);
        }

        private void ReplayTutorialFromSettings()
        {
            HideModal();
            StartTutorial(true);
        }

        private void StartTutorial(bool replay)
        {
            tutorialActive = true;
            tutorialStep = 0;
            tutorialPlotIndex = FirstUsableTutorialPlot();
            if (!replay && plots[tutorialPlotIndex] == PlotState.Locked)
            {
                plots[tutorialPlotIndex] = PlotState.Empty;
                plotTimers[tutorialPlotIndex] = 0f;
            }
            ShowHarvest(false);
            ShowPage(Page.Town);
            BuildTutorialStep();
        ...
        private int FirstUsableTutorialPlot()
        {
            for (var i = 0; i < plots.Length; i++)
            {
                if (plots[i] == PlotState.Empty) return i;
            }
            for (var i = 0; i < plots.Length; i++)
            {
                if (plots[i] != PlotState.Locked) return i;
            }
            return 0;
        }

        private void BuildTutorialStep()
        {
            if (!tutorialActive || guideLayer == null) return;
            guideLayer.gameObject.SetActive(true);
            guideLayer.SetAsLastSibling();
            Clear(guideLayer);

            var target = TutorialTargetRect(tutorialStep);
            DrawGuideMask(target);

            var halo = Panel(guideLayer, "SpotlightHalo", C(255, 231, 135, 56), target.center, target.size + new Vector2(18, 18), null);
            AddReadableBorder(halo, halo.sizeDelta, 3f, C(255, 226, 105, 245));
            var haloImage = halo.GetComponent<Image>();
            if (haloImage != null) haloImage.raycastTarget = false;

            var tap = ButtonNode(guideLayer, "GuideTargetTap", "", target.center, target.size, C(255, 255, 255, 1), RunTutorialStepAction, null);
            var tapImage = tap.GetComponent<Image>();
            if (tapImage != null) tapImage.color = C(255, 255, 255, 1);

            var cardY = target.center.y > 24 ? Mathf.Max(-330, target.yMin - 124) : Mathf.Min(338, target.yMax + 124);
            var card = Panel(guideLayer, "GuideCard", C(255, 250, 232, 255), new Vector2(0, cardY), new Vector2(366, 118), null);
            AddReadableBorder(card, new Vector2(366, 118), 3f, C(181, 137, 74, 230));
            guideText = TextNode(card, "Copy", TutorialCopy(tutorialStep), new Vector2(0, 16), new Vector2(326, 66), 14, C(45, 55, 40, 255), TextAnchor.MiddleCenter);
            guideText.overflowMode = TextOverflowModes.Overflow;
            TextNode(card, "Step", (tutorialStep + 1) + " / 8", new Vector2(0, -42), new Vector2(120, 22), 14, C(116, 91, 57, 255), TextAnchor.MiddleCenter);

            if (tutorialPulseRoutine != null) StopCoroutine(tutorialPulseRoutine);
            tutorialPulseRoutine = StartCoroutine(PulseTutorialTarget(halo));
        }

        private void DrawGuideMask(Rect target)
        {
            const float leftEdge = -215f;
            const float rightEdge = 215f;
            const float bottomEdge = -466f;
            const float topEdge = 466f;
            var xMin = Mathf.Clamp(target.xMin, leftEdge, rightEdge);
            var xMax = Mathf.Clamp(target.xMax, leftEdge, rightEdge);
            var yMin = Mathf.Clamp(target.yMin, bottomEdge, topEdge);
            var yMax = Mathf.Clamp(target.yMax, bottomEdge, topEdge);
            GuideShade("TopShade", new Rect(leftEdge, yMax, rightEdge - leftEdge, topEdge - yMax));
            GuideShade("BottomShade", new Rect(leftEdge, bottomEdge, rightEdge - leftEdge, yMin - bottomEdge));
            GuideShade("LeftShade", new Rect(leftEdge, yMin, xMin - leftEdge, yMax - yMin));
            GuideShade("RightShade", new Rect(xMax, yMin, rightEdge - xMax, yMax - yMin));
        }

        private void GuideShade(string name, Rect rect)
        {
            if (rect.width <= 0f || rect.height <= 0f) return;
            var shade = Panel(guideLayer, name, C(0, 0, 0, 205), rect.center, rect.size, null);
            var image = shade.GetComponent<Image>();
            if (image != null) image.raycastTarget = true;
        }

        private IEnumerator PulseTutorialTarget(RectTransform target)
        {
            while (tutorialActive && target != null)
            {
                var t = 0f;
                while (t < 1f && tutorialActive && target != null)
                {
                    t += Time.deltaTime * 2.6f;
                    target.localScale = Vector3.one * (1f + Mathf.Sin(t * Mathf.PI) * .06f);
                    yield return null;
                }
            }
        }

        private Rect TutorialTargetRect(int step)
        {
            switch (step)
            {
                case 0:
                    return RectFromCenter(PlotPosition(tutorialPlotIndex), new Vector2(116, 90));
                case 1:
                    return RectFromCenter(new Vector2(0, -349), new Vector2(242, 62));
                case 2:
                    return RectFromCenter(new Vector2(50, -427), new Vector2(98, 76));
                case 3:
                    return RectFromCenter(new Vector2(48, 292), new Vector2(92, 46));
                case 4:
                    return RectFromCenter(new Vector2(142, 292), new Vector2(92, 46));
                case 5:
                    return RectFromCenter(new Vector2(0, -300), new Vector2(240, 62));
                case 6:
                    return RectFromCenter(new Vector2(126, -122), new Vector2(136, 126));
                case 7:
                    return RectFromCenter(new Vector2(0, -194), new Vector2(132, 78));
                default:
                    return RectFromCenter(Vector2.zero, new Vector2(160, 80));
            }
        }

        private Vector2 PlotPosition(int index)
        {
            var col = index % 2;
            var row = index / 2;
            return new Vector2(-146 + col * 98, 230 - row * 84);
        }
        ...
        private string TutorialCopy(int step)
        {
            switch (step)
            {
                case 0:
                    return "Tap this plot first. This is where town production begins.";
                case 1:
                    return "Plant wheat here. Let it grow, then harvest it for orders.";
                case 2:
                    return "Open Tasks to see what the town needs and what each job pays.";
                case 3:
                    return "Album is now a top tab. Check collection progress here.";
                case 4:
                    return "Route tracks daily harvest rewards. More harvests unlock more stops.";
                case 5:
                    return "Go back to Town. Buildings, residents, and Harvest all start on the map.";
                case 6:
                    return "Harvest is the settlement game. It gives coins, wood, and materials.";
                case 7:
                    return "Tap Harvest once to finish the guide. After that, you can play freely.";
                default:
                    return "Start growing your town.";
            }
        }

        private void RunTutorialStepAction()
        {
            switch (tutorialStep)
            {
                case 0:
                    ClickPlot(tutorialPlotIndex);
                    AdvanceTutorial(1);
                    break;
                case 1:
                    PlantTutorialWheat();
                ...
                case 2:
                    ...
                    break;
                case 3:
                    activeTaskTab = TaskTab.Album;
                    BuildTaskPage();
                    AdvanceTutorial(4);
                    break;
                case 4:
                    activeTaskTab = TaskTab.DailyRoute;
                    BuildTaskPage();
                    AdvanceTutorial(5);
                    break;
                case 5:
        ...
        private void PlantTutorialWheat()
        {
            plots[tutorialPlotIndex] = PlotState.GrowingOne;
            plotTimers[tutorialPlotIndex] = 12f;
            HideModal();
            PlaySfx(SfxKind.Plant);
            Toast("Wheat planted");
            SaveGuestData();
        ...
        private void CompleteTutorial()
        {
            ...
            PlayerPrefs.Save();
            Toast("Guide complete");
        }

        private void PlaySfx(SfxKind kind, float volumeScale = 1f)
        {
            ...
            switch (kind)
            {
                ...
                case SfxKind.Plant:
                    clip = sfxPlant;
                    break;
                case SfxKind.HarvestStart:
                    clip = sfxHarvestStart;
                    break;
                case SfxKind.Reward:
                    clip = sfxReward;
                    break;
            ...
            if (clip != null) sfxSource.PlayOneShot(clip, Mathf.Clamp01(masterVolume * volumeScale));
        }

        private AudioClip CreateToneClip(string clipName, float startFrequency, float endFrequency, float duration, float volume, Wave wave)
        {
            ...
            {
                var t = sampleCount <= 1 ? 1f : (float)i / (sampleCount - 1);
                var frequency = Mathf.Lerp(startFrequency, endFrequency, t);
                phase += frequency / sampleRate;
                phase -= Mathf.Floor(phase);
                var waveValue = WaveValue(phase, wave);
                var attack = Mathf.Clamp01(t / .12f);
                var release = Mathf.Clamp01((1f - t) / .28f);
                var envelope = Mathf.Sin(Mathf.Clamp01(t) * Mathf.PI) * Mathf.Min(attack, release);
                data[i] = waveValue * envelope * volume;
            }
        ...
        private static float WaveValue(float phase, Wave wave)
        {
            switch (wave)
            {
                case Wave.Triangle:
                    return 4f * Mathf.Abs(phase - .5f) - 1f;
                case Wave.Square:
                    return phase < .5f ? 1f : -1f;
                case Wave.Saw:
                    return phase * 2f - 1f;
                default:
        ...
        {
            toastBox = Panel(root, "Toast", C(27, 52, 39, 245), new Vector2(0, -350), new Vector2(340, 44), null);
            AddReadableBorder(toastBox, new Vector2(340, 44), 2f, C(255, 231, 157, 210));
            toastText = TextNode(toastBox, "Text", "", Vector2.zero, new Vector2(318, 34), 16, Color.white, TextAnchor.MiddleCenter);
        ...
        private Toggle ToggleNode(Transform parent, string label, Vector2 pos)
        {
            ...
            rect.anchoredPosition = pos;
            // Keep the complete agreement control inside the parchment's safe content area.
            // The previous -144 offset placed the checkbox over the carved outer frame at 1290x2796.
            rect.sizeDelta = new Vector2(292, 42);
            var bg = Panel(rect, "Box", C(255, 255, 255, 255), new Vector2(-130, 0), new Vector2(26, 26), null);
            ...
            TextNode(rect, "Label", label, new Vector2(25, 0), new Vector2(250, 32), 15, C(55, 68, 55, 255), TextAnchor.MiddleLeft);
        ...
        private Sprite AuthTitlePlaque()
        {
            if (authTitlePlaqueRuntime != null) return authTitlePlaqueRuntime;
            authTitlePlaqueRuntime = CreateAuthTitlePlaqueSprite();
            return authTitlePlaqueRuntime;
        }

        private Sprite CreateAuthTitlePlaqueSprite()
        {
            const int width = 720;
            const int height = 236;
            var texture = new Texture2D(width, height, TextureFormat.RGBA32, false)
            {
                name = "AuthTitlePlaqueRuntime",
            ...
            for (var y = 0; y < height; y++)
            {
                ...
                {
                    var px = x - halfWidth + .5f;
                    var py = y - halfHeight + .5f;
                    var distance = RoundedRectDistance(px, py, halfWidth - 12f, halfHeight - 12f, 42f);
                    var alpha = Mathf.Clamp01(.5f - distance / 3f);
                    if (alpha <= 0f)
                    {
                    ...
                    var baseColor = Color.Lerp(C(18, 55, 36, 255), C(43, 91, 56, 255), vertical);
                    var warmCenter = Mathf.Clamp01(1f - Mathf.Abs(px) / (halfWidth * .92f)) * Mathf.Clamp01(1f - Mathf.Abs(py) / (halfHeight * .82f));
                    baseColor = Color.Lerp(baseColor, C(65, 111, 68, 255), warmCenter * .28f);
                    if (distance > -18f) baseColor = Color.Lerp(C(111, 72, 34, 255), C(237, 190, 89, 255), Mathf.Clamp01((distance + 18f) / 18f));
                    else if (distance > -30f) baseColor = Color.Lerp(baseColor, C(255, 226, 142, 255), .22f);
                    if (Mathf.Abs(py) > halfHeight - 42f && distance < -28f) baseColor = Color.Lerp(baseColor, C(255, 221, 132, 255), .16f);
        ...
        private static float RoundedRectDistance(float x, float y, float halfWidth, float halfHeight, float radius)
        {
        ...
        private void AddReadableBorder(RectTransform parent, Vector2 size, float thickness = 3f, Color? color = null)
        {
            ...
            DisableRaycast(Panel(parent, "BorderTop", lineColor, new Vector2(0, size.y * .5f - thickness * .5f), new Vector2(size.x, thickness), null));
            DisableRaycast(Panel(parent, "BorderBottom", lineColor, new Vector2(0, -size.y * .5f + thickness * .5f), new Vector2(size.x, thickness), null));
            DisableRaycast(Panel(parent, "BorderLeft", lineColor, new Vector2(-size.x * .5f + thickness * .5f, 0), new Vector2(thickness, size.y), null));
            DisableRaycast(Panel(parent, "BorderRight", lineColor, new Vector2(size.x * .5f - thickness * .5f, 0), new Vector2(thickness, size.y), null));
        }

        private static void DisableRaycast(RectTransform rect)
        {
            if (rect == null) return;
